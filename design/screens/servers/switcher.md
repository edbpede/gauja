...
Screen ID `servers/switcher`, phase 5.1. Entry and exit remain scoped to the selected profile. Auth surfaces are native forms/sheets except the explicitly external Plex system browser. Reference contract: Seerr v3.4.1 at `69f73a6f1486fdb51b8ddae9a94a8dfb629f461c`.

## Content

Named profiles with the current selection and Operator/HTTP indicators. Preserve unsubmitted drafts within their original profile, never retarget them.
...
Switch the active profile atomically, cancel previous profile work, restore the selected profile’s own cache/session and refresh status/public settings on foreground. Late results retain their original profile ID.
...
GET /status; GET /settings/public; GET /auth/me for the chosen profile as needed. Paths are relative to `/api/v1`. Request/response shapes come from the effective spec. Authenticated requests go to the profile’s Seerr origin only; cookies never cross profiles.

## Permissions

Local selection; rebuild visible destinations from the chosen user’s permissions and version gates.
