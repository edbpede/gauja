...
Cast, crew and search-result person identity. Consumed by Phase 4 and the owning feature phases. Reference: Seerr `src/components/PersonCard/` at `69f73a6f1486fdb51b8ddae9a94a8dfb629f461c` (inspiration only). Native chrome belongs to the containing screen.

## Content

Typed person ID, name, profile image, role/character when present and missing-image placeholder. Inputs are immutable domain values; state flows down and events flow up. Do not create transport, storage or navigation owners inside this renderer.
...
GET /person/{personId}; GET /person/{personId}/combined_credits via its screen. The owning Data/screen layer performs I/O and handles pagination; the component does not.
