...
Screen ID `auth/reauthenticate`, phase 5.2. Entry and exit remain scoped to the selected profile. Auth surfaces are native forms/sheets except the explicitly external Plex system browser. Reference contract: Seerr v3.4.1 at `69f73a6f1486fdb51b8ddae9a94a8dfb629f461c`.

## Content

Expired-session explanation, affected profile name and the previously selected sign-in method. Other profiles remain available.
...
A 401 invalidates only this profile’s active session and prompts re-authentication; never replay a failed mutation automatically. Logout clears that profile’s local cookie/key even if the server logout fails offline, with an explanation when remote invalidation is unconfirmed.
...
GET /auth/me; POST /auth/logout; the selected sign-in endpoint. Paths are relative to `/api/v1`. Request/response shapes come from the effective spec. Authenticated requests go to the profile’s Seerr origin only; cookies never cross profiles.

## Permissions

Permissions refresh after successful sign-in. Distinguish 401 from 403: a forbidden action is not automatically a lost session.
