...
Screen ID `auth/reset-request`, phase 5.2. Entry and exit remain scoped to the selected profile. Auth surfaces are native forms/sheets except the explicitly external Plex system browser. Reference contract: Seerr v3.4.1 at `69f73a6f1486fdb51b8ddae9a94a8dfb629f461c`.

## Content

Email field, target server name and a neutral completion message that does not disclose account existence.
...
Submit once; show the same user-facing completion wording for accepted requests. Handle connectivity/rate failures without exposing server internals. Password reset links open the completion flow when deep-link registration lands in Phase 11.
...
POST /auth/reset-password. Paths are relative to `/api/v1`. Request/response shapes come from the effective spec. Authenticated requests go to the profile’s Seerr origin only; cookies never cross profiles.

## Permissions

Local account/password recovery when the server enables email. No active session required.
