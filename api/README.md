...
`coverage.json` is editable implementation status, keyed by operation ID. Render the endpoint checklist to stdout with `tools/contract/python.sh tools/contract/endpoints.py`, or add `--output api/ENDPOINTS.md` for an ignored local report. The drift validator checks complete coverage independently of this report. No feature is implemented merely because its client exists. Excluded initialization, push-subscription and sunset blacklist operations must not be called by Gauja.
